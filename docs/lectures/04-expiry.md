...
- セッション管理: ユーザーの非アクティブ期間後に自動削除
- キャッシュ: 古いデータを自動的に無効化
- レート制限: 時間枠ごとにカウンターをリセット
- 一時トークン: ワンタイムパスワードやトークンの有効期限
...
参考: [公式ドキュメント](https://redis.io/docs/latest/commands/expire/)
...
参考: [公式ドキュメント](https://redis.io/docs/latest/commands/ttl/)
...
本ワークショップにおいて、Active Expiryは以下のアルゴリズムで動作します。
