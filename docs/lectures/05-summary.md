...
✅ **実装内容**:
- 5つのRESPデータ型のパース（Simple Strings, Errors, Integers, Bulk Strings, Arrays）
- コマンド配列のパース（`GET mykey` → `["GET", "mykey"]`）
- 応答のエンコード（Pythonオブジェクト → RESPバイト列）
...
✅ **実装内容**:
- `asyncio.start_server()`によるTCPサーバ構築
- StreamReader/StreamWriterによるデータ送受信
- 複数クライアントの並行処理
- クリーンアップとgraceful shutdown
...
| コマンド | 用途 | 学習ポイント |
|---------|------|-------------|
| **PING** | 接続確認 | 引数の有無で応答を変える |
| **GET** | 値取得 | Null値の扱い、Passive Expiry統合 |
| **SET** | 値設定 | シンプルなキー・バリュー操作 |
| **INCR** | インクリメント | 型チェック、エラーハンドリング |
| **EXPIRE** | 期限設定 | Unixタイムスタンプの計算 |
| **TTL** | 残り時間取得 | -1（期限なし）/-2（キー不在）の区別 |
...
✅ **実装内容**:
- **Passive Expiry**: コマンド実行時の期限チェック
- **Active Expiry**: バックグラウンドでのランダムサンプリング
- asyncioバックグラウンドタスク
...
Mini-Redisワークショップを通じて、以下のことを達成しました：
