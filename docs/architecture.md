...
Mini-Redisは、**レイヤー分離アーキテクチャ**を採用し、各レイヤーが明確な責務を持つことで、テスタビリティとメンテナンス性を向上させています。
...
**依存関係**:
- Protocol Layer: コマンドのパース・応答のエンコード
- Command Layer: コマンドの実行

**責務外**:
- RESPプロトコルの詳細（Protocol Layerに委譲）
- コマンドのビジネスロジック（Command Layerに委譲）
...
Mini-Redisのアーキテクチャは、以下の特徴を持ちます：

✅ **レイヤー分離**: 各レイヤーが明確な責務を持つ
✅ **一方向依存**: 上位層は下位層に依存、逆依存なし
